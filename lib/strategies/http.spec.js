...
test('createHttpStrategy: handles non-error object', async (t) => {
  const container = createContainer()
  container.register('processor', asValue(processorWithNonErrorObject))
  container.register('log', asValue(createLogger({ t })))
  const strategy = createHttpStrategy(container)
  const data = await strategy({ a: 1 }, { b: 2 })
  t.deepEqual(data, { statusCode: 500 })
...
test('createHttpStrategy: handles empty non-error object', async (t) => {
  const container = createContainer()
  container.register('processor', asValue(processorWithEmptyNonErrorObject))
  container.register('log', asValue(createLogger({ t })))
  const strategy = createHttpStrategy(container)
  const data = await strategy({ a: 1 }, { b: 2 })
  t.deepEqual(data, { statusCode: 500 })
...
const processorWithNonErrorObject = async (event, ctx) => {
  const err = new NonError()
  err.message = 'foo'
  throw err
}

const processorWithEmptyNonErrorObject = async (event, ctx) => {
  throw new NonError()
}
